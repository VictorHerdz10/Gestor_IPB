...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const extraccionesLinks = document.querySelectorAll('a[data-section="extracciones"]');
    extraccionesLinks.forEach(link => {
    ...
    function actualizarListaExtracciones() {
        ...
        extraccionesOrdenadas.forEach(extraccion => {
            ...
            if (extraccion.esAutomatico) {
                card.classList.add('extraccion-automatica');
            }
        ...
        ` : '';
            ...
            // NUEVO: Mostrar indicador de extracción automática
            const origenHTML = extraccion.esAutomatico ? `
            <div class="card-origen">
                <span class="badge badge-automatico">
                    <i class="fas fa-robot"></i> Automático
                </span>
            </div>
        ` : '';

            // NUEVO: Solo mostrar botón eliminar si NO es automático
            const accionesHTML = !extraccion.esAutomatico ? `
            <div class="acciones">
        ...
        ` : `
            <div class="acciones">
                <span class="text-muted" title="No se puede eliminar desde aquí">
        ...
        // Agregar event listeners para botones de eliminar (solo los no automáticos)
        setTimeout(() => {
            const botonesEliminar = document.querySelectorAll('.btn-eliminar:not(.disabled)');
    ...
    function eliminarExtraccion(id) {
        // NUEVO: Verificar si es extracción automática
        const extraccion = extracciones.find(ex => ex.id === id);

        if (extraccion && extraccion.esAutomatico) {
            showNotification('Las extracciones automáticas no se pueden eliminar desde aquí. Elimina el registro del conteo en la sección de billetes.', 'warning');
            return;
        }
